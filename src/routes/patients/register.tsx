...
interface PatientFormData {
  name: string;
  dateOfBirth?: string; // UPDATED: Use dateOfBirth instead of age
  gender?: string;
  phoneNumber: number;
  addressLine1?: string;
  city?: string;
...
interface SampleCollectionData {
  sampleCollectionTime?: string; // NEW: Time when sample was collected
  sampleCollectionPlace?: string; // NEW: Location where sample was collected
  sampleCollectorName?: string; // NEW: Name of person who collected sample
  sampleArrivalTime?: string; // NEW: Time when sample arrived at lab
...
interface TestFormData {
  ...
  discount?: string;
  tax?: string;
  sampleCollection?: SampleCollectionData; // NEW: Include sample collection data
}

function PatientRegistration() {
  const [step, setStep] = useState<'search' | 'register' | 'tests' | 'payment'>('search');
  const [phoneNumber, setPhoneNumber] = useState('');
  const [calculatedAge, setCalculatedAge] = useState<number | null>(null); // NEW: State for calculated age
  ...
  const {
    ...
    watch: watchPatient, // UPDATED: Add watch to track dateOfBirth
  } = useForm<PatientFormData>();

  const dateOfBirth = watchPatient('dateOfBirth');

  // UPDATED: Calculate age when dateOfBirth changes
  useEffect(() => {
    if (dateOfBirth) {
  ...
  const onRegisterPatient = async (data: PatientFormData) => {
    ...
    setExistingPatient({ 
      ...data, 
      age, // UPDATED: Include calculated age
      phoneNumber: parseInt(phoneNumber) 
    });
    setStep('tests');
  };
      ...
      const result = await createPatientWithTests({
        data: {
          ...
          patient: {
            ...
            dateOfBirth: existingPatient.dateOfBirth, // UPDATED: Send dateOfBirth
            age: existingPatient.age, // Age will be calculated if needed
          ...
          // NEW: Include sample collection data
          sampleCollection: {
            sampleCollectionTime: testFormData.sampleCollectionTime,
            sampleCollectionPlace: testFormData.sampleCollectionPlace,
            sampleCollectorName: testFormData.sampleCollectorName,
            sampleArrivalTime: testFormData.sampleArrivalTime,
            ...
            <form onSubmit={handleSubmitPatient(onRegisterPatient)}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {errorsPatient.dateOfBirth && (
                    <p className="text-red-600 text-xs mt-1">
                      {errorsPatient.dateOfBirth.message}
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full px-3 py-2 border border-gray-300 text-sm bg-gray-100 rounded font-semibold text-gray-700"
                  />
                </div>
        ...
        {!isLoadingData && step === 'tests' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {/* NEW: Sample Collection Details Section */}
              <div className="bg-white border border-gray-300 p-4">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="datetime-local"
                          {...registerTests('sampleCollectionTime', {
                            required: 'Sample collection time is required',
                          })}
                          className="w-full px-3 py-2 border border-gray-300 text-sm rounded"
                        />
                      </div>
                      ...
                      <div>
                        <Label className="block text-sm font-medium text-gray-900 mb-1">
                          Sample Arrival Time at Lab *
                        </Label>
                        <input
                          type="datetime-local"
                          {...registerTests('sampleArrivalTime', {
                            required: 'Sample arrival time is required',
                          })}
                          className="w-full px-3 py-2 border border-gray-300 text-sm rounded"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="text"
                          {...registerTests('sampleCollectionPlace', {
                            required: 'Sample collection place is required',
                          })}
                          className="w-full px-3 py-2 border border-gray-300 text-sm rounded"
                          placeholder="e.g., Home, Lab, Hospital, Clinic"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          {...registerTests('sampleCollectorName', {
                            required: 'Sample collector name is required',
                          })}
                          className="w-full px-3 py-2 border border-gray-300 text-sm rounded"
            ...
            <div className="bg-white border border-gray-300 p-8 rounded-lg">
              ...
              <div className="bg-gray-50 border border-gray-300 p-6 rounded-lg mb-6">
                ...
                <div className="space-y-3">
                  ...
                  {/* NEW: Sample Collection Details Display */}
                  {testFormData && (
                    <>
                      <div className="border-t border-gray-300 pt-3 mt-3">
                        <p className="text-sm font-semibold text-gray-900 mb-2">Sample Collection Details:</p>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span className="text-gray-600">Collection Time:</span>
                            <span className="text-gray-900">{testFormData.sampleCollectionTime ? new Date(testFormData.sampleCollectionTime).toLocaleString() : 'N/A'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Collection Place:</span>
                            <span className="text-gray-900">{testFormData.sampleCollectionPlace || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Collector Name:</span>
                            <span className="text-gray-900">{testFormData.sampleCollectorName || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-600">Arrival Time:</span>
                            <span className="text-gray-900">{testFormData.sampleArrivalTime ? new Date(testFormData.sampleArrivalTime).toLocaleString() : 'N/A'}</span>
                          </div>
                          {testFormData.notes && (
                            <div className="flex justify-between">
                              <span className="text-gray-600">Notes:</span>
                              <span className="text-gray-900 text-right">{testFormData.notes}</span>
                            </div>
                          )}
                        </div>
                      </div>
